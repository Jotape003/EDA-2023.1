...
{
    size_t cont = hash_code(k);
    for(auto& v : (*_table)[cont]){
        if(v.first == k){
            return 1;
        }
    }
...
{
    if (n > (_table_size * _load_factor)) {
        rehash(n/_load_factor);
    }
}
...
HashTable<K,V>::HashTable(const HashTable& t) 
{
    ...
    _load_factor = t.load_factor();
    _max_load_factor = t.max_load_factor();
    _hashing = t._hashing;
    _table = new std::vector<std::list<std::pair<K, V>>>;
    _table->resize(_table_size);
    _n = t._n;
