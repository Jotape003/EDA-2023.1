...
int Tree::_sum_keys(Node *node) { // TODO
    if(node == nullptr){
        ...
        int soma = node->key;
        soma += _sum_keys(node->left) + _sum_keys(node->right);
        return soma;
    }
}
...
int Tree::_min_key(Node *node) { // TODO
    ...
    if (node == nullptr){
        ...
        if (node->left != nullptr){
            minimoE = _min_key(node->left);
        }
        if (node->right != nullptr){
            minimoD = _min_key(node->right);
        }

        if (minimoE < node->key && minimoE < minimoD){
            return minimoE;
        }
        else if (minimoD < node->key && minimoD < minimoE){
            return minimoD;
        }
        else {
            return node->key;
